...
class NNClassifier(nn.Module):
    def __init__ (self,input,hidden,output):
        ...
        self.fc3 = nn.Linear(10, 11)
        # Non-linearity 3
        #self.elu3 = nn.ELU()
        self.relu3 = nn.ReLU()
        # Linear function 4 (readout): 150 --> 10

        self.fc4 = nn.Linear(11, 12)
        self.relu4 = nn.ReLU()  

        self.fc5 = nn.Linear(12, 13)
        self.relu5 = nn.ReLU()  

        self.fc6 = nn.Linear(13,14 )
        self.relu6 = nn.ReLU()  
        
        self.fc7 = nn.Linear(14, output_dim)  
...
for epoch in range(num_epochs):
    for i in range(0,len(X_train)):
        ...
        if count % 50 == 0:
            # Calculate Accuracy
            # learning_rate =  random.randrange(1,10)/1000
            # optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate)
...
make_dot(yhat, params=dict(list(model.named_parameters()))).render("NNC2", format="png")

# Saving & Loading Model for Inference
# Save/Load state_dict (Recommended)

# Save:
torch.save(model.state_dict(), "PTTL/NNC2.pt")

# Load:
Loded_model = NNClassifier(input_dim,hidden_dim,output_dim)
Loded_model.load_state_dict(torch.load("PTTL/NNC2.pt"))
Loded_model.eval()
